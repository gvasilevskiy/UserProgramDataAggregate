...
import com.marsh.itg.ppm.dashboard.config.KafkaProperties;
...
class QuickUserProgramDataAggregateKafkaResourceIT {
    ...
    void setup() {
        ...
        QuickUserProgramDataAggregateKafkaResource kafkaResource = new QuickUserProgramDataAggregateKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/quick-user-program-data-aggregate-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/quick-user-program-data-aggregate-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
